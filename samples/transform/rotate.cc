...
void MyRenderWindow::OnInit() {
  ...
  Shaders programs;
  CHECK(LoadStageShader(kVertexStage, "demo/base/hlsl/tex.vs.hlsl", &programs));
  CHECK(LoadStageShader(kPixelStage, "demo/base/hlsl/tex.ps.hlsl", &programs));
...
void MyRenderWindow::OnUpdateFrame(const FrameArgs& args) {
  world_ = RotateY(Radians(args.time() * 3.14f))
